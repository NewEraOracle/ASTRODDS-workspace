...
$script238 = Join-Path $scripts "238_run_final_live_official_scan.ps1"
$script240 = Join-Path $scripts "240_final_trusted_full_slate_gate.ps1"
$script241 = Join-Path $scripts "241_log_trusted_full_slate_officials.ps1"
$script242 = Join-Path $scripts "242_clean_official_ledger_dedupe.ps1"
$script244 = Join-Path $scripts "244_build_simple_confidence_telegram.ps1"

$outTxt = Join-Path $astro "ASTRODDS-final-confidence-client-drop-run-NO-243-latest.txt"
$outJson = Join-Path $astro "ASTRODDS-final-confidence-client-drop-run-NO-243-latest.json"
$outChildLog = Join-Path $astro "ASTRODDS-final-confidence-client-drop-child-log-NO-243-latest.txt"
$clientMessage = Join-Path $astro "ASTRODDS-simple-confidence-official-message-latest.txt"

Write-Host ""
Write-Host "ASTRODDS 245B FINAL CONFIDENCE CLIENT DROP RUNNER" -ForegroundColor Cyan
Write-Host "POWERSHELL ONLY - NO 243 DEPENDENCY" -ForegroundColor Cyan
Write-Host ""
...
$steps = @()
$steps += ,(Run-Step "238 final live official scan" $script238)
$steps += ,(Run-Step "240 trusted full slate gate" $script240)
$steps += ,(Run-Step "241 log trusted full slate officials" $script241)
$steps += ,(Run-Step "242 clean official ledger dedupe" $script242)
...
$lines = @()
$lines += "ASTRODDS 245B FINAL CONFIDENCE CLIENT DROP RUNNER"
$lines += ""
