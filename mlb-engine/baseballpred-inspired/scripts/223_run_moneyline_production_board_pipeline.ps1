...
function Run-Step($Label, $FileName, $IsPowerShell = $false) {
  ...
  if ($IsPowerShell) {
    $p = Start-Process powershell -ArgumentList "-ExecutionPolicy Bypass -File `"$path`" -Workspace `"$Workspace`"" -WorkingDirectory $Workspace -NoNewWindow -Wait -PassThru
  } else {
...
$codes += Run-Step "Moneyline actionable guard 222" "222_moneyline_actionable_guard.py"
