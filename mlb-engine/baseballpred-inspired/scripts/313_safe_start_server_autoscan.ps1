...
$outTxt = Join-Path $astro "ASTRODDS-313-safe-start-server-autoscan-latest.txt"

Write-Host ""
Write-Host "ASTRODDS 313 SAFE START SERVER + AUTOSCAN" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path (Join-Path $root "package.json")) {
    ...
    Write-Host "No package.json found. Server not started." -ForegroundColor Yellow
}

Start-Sleep -Seconds 3

$loopCmd = "cd `"$root`"; `$env:ASTRODDS_SCAN_INTERVAL_MINUTES='10'; powershell -ExecutionPolicy Bypass -File `"$scripts\308_final_production_one_command.ps1`""
Start-Process powershell -ArgumentList "-NoExit","-Command",$loopCmd

$lines = @()
$lines += "ASTRODDS 313 SAFE START SERVER + AUTOSCAN"
$lines += ""
$lines += "Server started: $serverStarted"
$lines += "Server command: $serverCmd"
$lines += "One production scan launched in separate PowerShell."
$lines += ""
$lines += "For loop mode, use 311 Task Scheduler or run manually every 10 min."
