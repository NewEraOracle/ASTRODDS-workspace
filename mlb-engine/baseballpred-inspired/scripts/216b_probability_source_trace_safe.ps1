...
# SAFE OUTPUT BLOCK - avoids nested Generic List crash

$traceRows | Export-Csv -NoTypeInformation -Encoding UTF8 $outCsv

$rootCauseArray = @()
foreach ($rc in @($rootCauses)) {
    $rootCauseArray += [pscustomobject]@{
        Pick = "$($rc.Pick)"
        Game = "$($rc.Game)"
        Causes = @($rc.Causes | ForEach-Object { "$_" })
    }
}

$traceArray = @()
foreach ($tr in @($traceRows)) {
    $traceArray += [pscustomobject]@{
        Source = "$($tr.Source)"
        Game = "$($tr.Game)"
        Pick = "$($tr.Pick)"
        Found = "$($tr.Found)"
        Status = "$($tr.Status)"
        PaperOnly = "$($tr.PaperOnly)"
        TelegramEligible = "$($tr.TelegramEligible)"
        Market = "$($tr.Market)"
        Model = "$($tr.Model)"
        CalibratedModel = "$($tr.CalibratedModel)"
        Edge = "$($tr.Edge)"
...
$resultSafe = [ordered]@{
    ...
    rootCauses = $rootCauseArray
    traceRows = $traceArray
    recommendation = "Do not send these picks as official until paperOnly/model mismatch/ranker model issues are fixed."
}

$resultSafe | ConvertTo-Json -Depth 20 | Set-Content -Encoding UTF8 $outJson
