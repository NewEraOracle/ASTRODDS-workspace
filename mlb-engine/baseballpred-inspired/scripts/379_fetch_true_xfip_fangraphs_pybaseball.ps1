...
$outCsv = Join-Path $astro "ASTRODDS-premium-input-starter-xfip.csv"
$outTxt = Join-Path $astro "ASTRODDS-379-fetch-true-xfip-fangraphs-pybaseball-latest.txt"
$outJson = Join-Path $astro "ASTRODDS-379-fetch-true-xfip-fangraphs-pybaseball-latest.json"
$py = Join-Path $scripts "379_fetch_true_xfip_fangraphs_pybaseball.py"

Write-Host ""
Write-Host "ASTRODDS 379F TRUE xFIP FETCH - EMPTY CSV REFRESH FIX" -ForegroundColor Cyan
Write-Host ""

$existingRows = Count-RealRows $outCsv @("Pitcher","xFIP")
if ((Is-FreshTodayWithRows $outCsv @("Pitcher","xFIP")) -and -not $Force) {
...
$rows = Count-RealRows $outCsv @("Pitcher","xFIP")
$lines = @(
    "ASTRODDS 379F TRUE xFIP FETCH - EMPTY CSV REFRESH FIX",
    "",
    "Status: $status",
    "Rows: $rows",
    "Output: $outCsv",
    ...
    "- xFIP values must come from real FanGraphs/pybaseball output.",
    "- FIP proxy is not renamed to xFIP."
)
