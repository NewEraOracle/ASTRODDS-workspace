...
$heartbeat = Join-Path $astro "ASTRODDS-339-autopilot-loop-heartbeat-latest.txt"
$loopLog = Join-Path $astro "ASTRODDS-339-autopilot-loop-log-latest.txt"

Write-Host ""
Write-Host "ASTRODDS 339F KEEPALIVE SERVER AUTOPILOT LOOP FIXED" -ForegroundColor Cyan
...
while ($true) {
    ...
    @(
        "ASTRODDS 339F AUTOPILOT LOOP HEARTBEAT",
        "Status: RUNNING",
    ...
    $out = & powershell -ExecutionPolicy Bypass -File (Join-Path $scripts "338_autopilot_server_scan_cycle.ps1") 2>&1
    $exit = $LASTEXITCODE
    ...
    @(
        ...
        "Run: $runs",
        "LastExit: $exit",
        "NextRunApprox: $($finished.AddMinutes($IntervalMinutes).ToString('o'))"
    ) | Set-Content -Encoding UTF8 $heartbeat
