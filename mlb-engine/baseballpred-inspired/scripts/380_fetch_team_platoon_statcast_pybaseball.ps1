param([switch]$Force,[int]$DaysBack = 30)
...
$outCsv = Join-Path $astro "ASTRODDS-premium-input-team-platoon-splits.csv"
$outTxt = Join-Path $astro "ASTRODDS-380-fetch-team-platoon-statcast-pybaseball-latest.txt"
$outJson = Join-Path $astro "ASTRODDS-380-fetch-team-platoon-statcast-pybaseball-latest.json"
$py = Join-Path $scripts "380_fetch_team_platoon_statcast_pybaseball.py"

Write-Host ""
Write-Host "ASTRODDS 380F TEAM PLATOON FETCH - EMPTY CSV REFRESH FIX" -ForegroundColor Cyan
Write-Host ""

$existingRows = Count-RealRows $outCsv @("Team","VsHand","OPS")
if ((Is-FreshTodayWithRows $outCsv @("Team","VsHand","OPS")) -and -not $Force) {
    ...
    $result = & $pythonCmd $py --days-back $DaysBack --out $outCsv 2>&1
    $exit = $LASTEXITCODE
    $status = if ($exit -eq 0) { "OK" } else { "ERROR" }
    $msg = ($result -join [Environment]::NewLine)
}

$rows = Count-RealRows $outCsv @("Team","VsHand","OPS")
$lines = @(
    "ASTRODDS 380F TEAM PLATOON FETCH - EMPTY CSV REFRESH FIX",
    "",
    "Status: $status",
    "DaysBack: $DaysBack",
    "Rows: $rows",
    ...
    "- OPS/OBP/SLG/K/BB are calculated from real Statcast events.",
    "- wRC+ is not invented; it stays blank unless a real external export provides it."
)
